...
function getFinals(data) {
  const getFinalTeams = data.filter((data) => {
...
function getYears(data, getFinalsCB) {
  const years = getFinalsCB(data).map(function (match) {
    return match.Year;
  });
  return years;
}
...
function getWinners(data, getFinalsCB) {
  const winners = getFinalsCB(data).map(function (match) {
...
function getWinnersByYear(data, getYearsCB, getWinnersCB) {
  const winnerStrings = getWinnersCB(data, getFinals);
  return getYearsCB(data, getFinals).map((year, i) => {
    return `In ${year}, ${winnerStrings[i]} won the world cup!`;
  });
}
